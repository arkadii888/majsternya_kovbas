...
export default function Aurora(props: AuroraProps) {
  ...
  const latestRef = useRef<AuroraProps>(props);
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColorStops: {
          value: (initial.colorStops ?? DEFAULT_STOPS).map(toVec3),
        },
        uResolution: { value: [ctn.offsetWidth, ctn.offsetHeight] },
        uBlend: { value: initial.blend ?? 0.5 },
        uLightMode: { value: (initial.lightMode ?? false) ? 1 : 0 },
      },
    });
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uLightMode.value = (p.lightMode ?? false) ? 1 : 0;
      program.uniforms.uColorStops.value = (p.colorStops ?? DEFAULT_STOPS).map(toVec3);
